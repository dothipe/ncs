...
export const SettingsPanel: React.FC<SettingsPanelProps> = ({
  ...
  const [newSoloShotCount, setNewSoloShotCount] = useState<number | "">("");
  ...
  const [newTeamSoloShotCount, setNewTeamSoloShotCount] = useState<number | "">("");
  ...
  const [editingSoloShotCount, setEditingSoloShotCount] = useState<number | "">("");
  ...
  const handleAddDistance = () => {
    ...
    const newDist: DistanceConfig = {
      ...
      isSolo: newIsElimination ? newIsSolo : false,
      soloShotCount: (newIsElimination && newIsSolo && Number(newSoloShotCount) > 0) ? Number(newSoloShotCount) : undefined,
    };
  ...
  const handleAddTeamDistance = () => {
    ...
    const newDist: DistanceConfig = {
      ...
      soloShotCount: (newTeamIsElimination && newTeamIsSolo && Number(newTeamSoloShotCount) > 0) ? Number(newTeamSoloShotCount) : undefined,
    };
      ...
      {tournamentType !== "team" && (
                ...
                <div className="flex flex-col gap-2.5 animate-fadeIn w-full">
                  ...
                  <div className="flex flex-col gap-1.5 mt-0.5">
                    ...
                    {editingIsElimination && (
                      <div className="flex flex-col gap-1.5 ml-4.5">
                        <label className="flex items-center gap-1.5 cursor-pointer text-xs font-semibold text-amber-700 dark:text-amber-405 select-none animate-fadeIn">
                          <input
                            type="checkbox"
                            checked={editingIsSolo}
                            onChange={(e) => {
                              const checked = e.target.checked;
                              setEditingIsSolo(checked);
                              if (!checked) setEditingSoloShotCount("");
                            }}
                        ...
                        {editingIsSolo && (
                          <div className="flex items-center gap-1.5 ml-5 animate-slideDown">
                            <span className="text-[10px] text-amber-800 dark:text-amber-300">
                              {language === "en" ? "Solo Shots (opt):" : "Số viên SOLO (t.chọn):"}
                            </span>
                            <input
                              type="number"
                              placeholder={language === "en" ? `Default` : `Mặc định`}
                              value={editingSoloShotCount}
                              onChange={(e) => {
                                const val = e.target.value;
                                setEditingSoloShotCount(val === "" ? "" : Math.max(1, Number(val)));
                              }}
                              className="w-24 px-1.5 py-0.5 text-xs bg-white dark:bg-slate-900 border border-amber-300 dark:border-slate-700 rounded focus:outline-none focus:ring-1 focus:ring-amber-500 font-mono text-amber-900 dark:text-amber-100"
                            />
                          </div>
                        )}
                      </div>
                          ...
                          const updated = distances.map((d) => {
                            if (d.id === editingDistanceId) {
                              return {
                                ...
                                soloShotCount: (editingIsElimination && editingIsSolo && Number(editingSoloShotCount) > 0) ? Number(editingSoloShotCount) : undefined,
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    <button
                      ...
                      onClick={() => {
                        ...
                        setEditingSoloShotCount(dist.soloShotCount || "");
        ...
        <div className="border-t border-gray-100 pt-3 flex flex-col gap-2.5">
          ...
          <div className="flex flex-col gap-1.5 mt-0.5">
            ...
            {newIsElimination && (
              <div className="flex flex-col gap-1.5 ml-4.5">
                <label className="flex items-center gap-1.5 cursor-pointer text-xs font-semibold text-amber-700 dark:text-amber-400 select-none animate-fadeIn">
                  <input
                    type="checkbox"
                    checked={newIsSolo}
                    onChange={(e) => {
                      const checked = e.target.checked;
                      setNewIsSolo(checked);
                      if (!checked) setNewSoloShotCount("");
                    }}
                    className="rounded border-amber-300 text-amber-600 focus:ring-amber-500"
                  />
                  <span>{language === "en" ? "Generate SOLO round on tie break" : "Phát sinh vòng SOLO khi bằng điểm"}</span>
                ...
                {newIsSolo && (
                  <div className="flex items-center gap-1.5 ml-5 animate-slideDown">
                    ...
                    <input
                      type="number"
                      placeholder={language === "en" ? `Default` : `Mặc định`}
                      value={newSoloShotCount}
                      onChange={(e) => {
                        const val = e.target.value;
                        setNewSoloShotCount(val === "" ? "" : Math.max(1, Number(val)));
                      }}
                      className="w-24 px-1.5 py-0.5 text-xs bg-white dark:bg-slate-900 border border-amber-300 dark:border-slate-700 rounded focus:outline-none focus:ring-1 focus:ring-amber-500 font-mono text-amber-900 dark:text-amber-100"
                    />
                  </div>
        ...
        <div className="border-t border-gray-100 pt-3 flex flex-col gap-2.5">
          ...
          <div className="flex flex-col gap-1.5 mt-0.5">
            ...
            {newTeamIsElimination && (
              <div className="flex flex-col gap-1.5 ml-4.5 font-sans">
                <label className="flex items-center gap-1.5 cursor-pointer text-xs font-black text-amber-700 dark:text-amber-400 select-none animate-fadeIn">
                  <input
                    type="checkbox"
                    checked={newTeamIsSolo}
                    onChange={(e) => {
                      const checked = e.target.checked;
                      setNewTeamIsSolo(checked);
                      if (!checked) setNewTeamSoloShotCount("");
                    }}
                ...
                {newTeamIsSolo && (
                  <div className="flex items-center gap-1.5 ml-5 animate-slideDown">
                    ...
                    <input
                      type="number"
                      placeholder={language === "en" ? `Default` : `Mặc định`}
                      value={newTeamSoloShotCount}
                      onChange={(e) => {
                        const val = e.target.value;
                        setNewTeamSoloShotCount(val === "" ? "" : Math.max(1, Number(val)));
                      }}
                      className="w-24 px-1.5 py-0.5 text-xs bg-white dark:bg-slate-900 border border-amber-300 dark:border-slate-700 rounded focus:outline-none focus:ring-1 focus:ring-amber-500 font-mono text-amber-900 dark:text-amber-100"
                    />
                  </div>
          ...
          <div className="relative w-full max-w-xl bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-3xl shadow-2xl overflow-hidden animate-scaleIn text-slate-800 dark:text-slate-100 flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 space-y-4 overflow-y-auto flex-1">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 items-end animate-fadeIn">
                <div 
                  onMouseEnter={() => setHoveringIsNational(true)}
                  onMouseLeave={() => setHoveringIsNational(false)}
                  className="flex items-center gap-2 bg-slate-50 dark:bg-slate-950 p-2.5 rounded-xl border border-gray-150 dark:border-slate-800 h-[38px] cursor-help"
                >
                  <input
                    type="checkbox"
                    id="modalIsNational"
