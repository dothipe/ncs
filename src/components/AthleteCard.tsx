              ...
              return distances.map((distance, distIdx) => {
                ...
                const rounds: (boolean | null)[][] = distance.isElimination && distance.isSolo && Array.isArray(rawRoundsArr) && rawRoundsArr.length > 0
                  ? rawRoundsArr.map(r => {
                      ...
                      if (typeof r === 'number') return Array.from({ length: soloShots }, (_, i) => i < r);
                      return Array.from({ length: soloShots }, () => null);
                    })
                  : [Array.from({ length: soloShots }, () => null)];
                      ...
                      return (
                        ...
                        >
                              ...
                              <div className="flex items-center gap-1 shrink-0">
                                {rIdx === 0 && (
                                 ...
                                 {rIdx === rounds.length - 1 && !isLocked && (
                                  <button
                                    onClick={() => {
                                      onUpdateSoloHits?.(athlete.id, distance.id, [...rounds, Array.from({ length: soloShots }, () => null)]);
                                    }}
                          ...
                          {isLocked ? (
                            ...
                            <>
                              {Array.from({ length: soloShots }).map((_, shotIdx) => {
                              ...
                              return (
                                <td 
                                  ...
                                  className={`text-center p-1 transition-colors relative border-r border-purple-200 dark:border-purple-900/40 bg-purple-50/20 dark:bg-purple-950/10 cursor-pointer hover:bg-purple-100/50 ${
                                    shotIdx === soloShots - 1 && soloShots === maxShots ? "" : "border-r"
                                  }`}
                            ...
                            {soloShots < maxShots && Array.from({ length: maxShots - soloShots }).map((_, padIdx) => (
                              <td 
                                key={`pad-solo-${padIdx}`}
                                className={`bg-purple-50/5 dark:bg-purple-950/5 border-r border-purple-200 dark:border-purple-900/40 ${
                                  soloShots + padIdx === maxShots - 1 ? "" : "border-r"
                                }`}
                              />
                            ))}
                          </>
